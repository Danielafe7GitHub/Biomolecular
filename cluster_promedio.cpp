...
void min_cluster()
{
    ...
    //Unimos menor distancia (debajo de la diagonal) solo para ejemplo
    //min_element();

    for(int i=0;i<831;i++) // i<6 para Ejemplo i<831 para .txt
    {
        ...
        for(int j=0;j<matriz_distancia.size();j++)
        {
            ...
            {
                //El único cambio (promedio)
                value = ((matriz_distancia[j][indx_n]) + (matriz_distancia[j][indx_m])) / 2;
